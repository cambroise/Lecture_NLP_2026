...
**Cross-attention** (encoder-decoder models): the decoder's Q comes from the decoder hidden state, while K and V come from the encoder output.
This is the original architecture of "Attention is All You Need" (Vaswani et al., 2017).
...
-   Handles unknown words (OOV) via decomposition.
...
![ChatGPT fine tuning  (from A. Karpathy lecture)](../images/chatGPT-steps.jpeg){height=70%}

The modern fine-tuning pipeline consists of three stages:

1. **Pre-training** — CLM on a large corpus; the model learns general language structure.
2. **Supervised Fine-Tuning (SFT)** — fine-tune on curated (prompt, response) pairs to teach the desired format and style.
3. **Reinforcement Learning from Human Feedback (RLHF)** — a reward model is trained on human preference rankings; the LLM policy is then optimised with PPO to maximise reward while staying close to the SFT model (KL penalty).
...
::: {.callout-tip}
Read the notebook alongside this chapter — each class maps directly to a formula above.
:::
